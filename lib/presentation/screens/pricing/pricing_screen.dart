...
class PricingScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tumUrunler = orderProv.pozlar.expand((poz) => poz.hesaplananUrunler).toList();

    return GestureDetector(
      // Klavye açık kalmasın diye kilit önlemi
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        ...
                        Text('Toplam Ağırlık: ${orderProv.pozlar.fold(0.0, (sum, p) => sum + p.hesaplananUrunler.fold(0.0, (s, u) => s + (u.kilogram * u.miktar))).toStringAsFixed(2)} kg'),
                      ],
                    ),

                    // İskonto Girişi (Rakam koruması ve updateGlobalDiscount ile güncellendi)
                    SizedBox(
                      width: 200,
                      child: FormFieldWrap(
                        etiket: 'Genel İskonto (%)',
                        child: TextFormField(
                          initialValue: orderProv.genelIskonto.toString(),
                          ...
                          onChanged: (val) {
                            final parsedVal = double.tryParse(val) ?? 0;
                            // BAĞLANTI BURADA YAPILDI: updateGlobalDiscount çağrılıyor
                            orderProv.updateGlobalDiscount(parsedVal);
                          },
                        ),
                      ),
                    ),
                    ...
                    Column(
                      ...
                      children: [
                        Text('Alt Toplam: ₺${orderProv.toplamMaliyet.toStringAsFixed(2)}', style: const TextStyle(color: AppColors.textMuted)),
                        const SizedBox(height: 4),
                        Text(
                          'Genel Toplam: ₺${(orderProv.toplamMaliyet * (1 - (orderProv.genelIskonto / 100))).toStringAsFixed(2)}',
